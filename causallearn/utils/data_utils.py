...
def my_simulate_linear_gaussian(W, K, n, sem_type):
    ...
    choice = np.random.choice(d, 2, replace=False) #replace参数意味着是否是有放回的
    # choice = [1]
    ...
    for j in ordered_vertices:
        parents = G.neighbors(j, mode=ig.IN)
        if j in choice: # changes
            sigma = np.random.uniform(low=1, high=3, size=K) #选定会变的变量在每个客户端加入一个随机系数
            sigma = np.repeat(sigma, int(n/K))
            ...
            var = np.random.uniform(low=1.0, high=2.0) # 选定会变的变量加入一个随机的方差，来控制异质性的程度。
            eps = np.random.normal(0, var, size=n)  #选定会变的变量加入一个随机的方差，来控制异质性的程度。每个客户端的方差不同，来体现异质性。
