...
# 计算协方差矩阵的函数，输入是两个矩阵X和Y
# 如果Y为None，则计算X的协方差矩阵；
# 如果Y不为None，则计算X和Y之间的协方差矩阵。
# 函数首先对输入矩阵进行中心化处理，即减去每列的均值，然后通过矩阵乘法计算协方差矩阵，并除以样本数量来得到最终的协方差矩阵。
...
def cdnod_alg(data: ndarray, c_indx: ndarray, alpha: float, K: int, indep_test: str, stable: bool, uc_rule: int, uc_priority: int,
    ...
    indep_test : name of the independence test being used
           ...
           - "Chi_sq": Chi-squared conditional independence test  卡方条件独立检验
           - "G_sq": G-squared conditional independence test
           - "MV_Fisher_Z": Missing-value Fishers'Z conditional independence test  适用于缺失值数据的 Fisher-Z 检验
           - "kci": kernel-based conditional independence test (If C is time index, KCI test is recommended) 基于核方法的条件独立检验
    ...
    # s_a is the number of samples, d is the number of variables (including surrogate variable c_indx)
