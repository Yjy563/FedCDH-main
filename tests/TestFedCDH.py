...
def test_fedCHD(i, n, K, d, s0, model):  
    #d: number of variables, s0: expected number of edges in the DAG
    set_random_seed(i)
    print(f"Runing instance {i}.")

    c_indx = np.asarray(list(range(K)))
    # asarray和array的区别：asarray不会复制输入数据，而array会复制输入数据。这里c_indx是一个长度为K的一维数组，表示每个样本所属的客户端索引。
    c_indx = np.repeat(c_indx, n) 
    # c_indx是一个长度为n*K的一维数组，其中每个元素的值表示对应样本所属的客户端索引。例如，如果K=3，n=2，那么c_indx将是[0, 0, 1, 1, 2, 2]，表示前两个样本属于客户端0，接下来的两个样本属于客户端1，最后两个样本属于客户端2。
    c_indx = np.reshape(c_indx, (n*K,1)) 
    #从向量变成矩阵
    graph_type, sem_type = 'ER', 'gauss'
    #设定图类型为 ER 随机图，噪声类型为高斯噪声。
    ...
    #通过特定规则，将混合图进一步转化为纯粹的 DAG（有向无环图），以便与真实的 DAG 进行比对
